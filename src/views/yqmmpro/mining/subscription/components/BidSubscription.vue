            ...
            <div class="defaultarea-slot-desc">
              ...
              >
                ...
                <a-menu slot="overlay">
                  <a-menu-item>
                    <img
                      src="@/assets/images/common/subscription-bid-qrcode.png"
                      alt=""
                    />
                  </a-menu-item>
                </a-menu>
...
<script>
  ...
  import { getBidSubscriptionList } from '@/api/yqmmPro/subscription'
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['bidTypeDict', 'cityDict']),
      // 筛选面板数据
      spLayout() {
        let spLayout = [
      ...
      /**已选筛选数量 */
      resultNum() {
        const num = Object.values(this.screenResults).filter(
          item => item
        ).length
    ...
    methods: {
      ...
      async handleQuery(opt) {
        ...
        if (res.code === '0') {
          this.bidList = JSON.parse(JSON.stringify(res.data.list))
          this.dataSource = JSON.parse(JSON.stringify(res.data.list))
          ...
          if (opt?.init) {
            if (res?.data?.searchList?.titleList?.length) {
              this.subscriptionTypeList = res.data.searchList.titleList
                .slice(0, 60)
                .map(e => {
                  return {
                    id: Object.keys(e)[0],
