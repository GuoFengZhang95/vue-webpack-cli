  ...
  <div ref="screenPanel" class="screen-panel-wrp">
    <div class="screen-panel_inner" :class="{ fold: !isExpanded }">
      <div class="screen-tr-list-wrp">
        <div
          v-for="(item, index) in renderLayout"
          :key="index"
          class="screen-tr-list-item"
        >
  ...
  export default {
    ...
    watch: {
      layout: {
        handler(newVal) {
          if (newVal.length) {
            let layoutClone = this.$lodash.cloneDeep(newVal)
    ...
    mounted() {
      ...
      this.$store.dispatch(
        'app/setScreenPanelRect',
        this.$refs['screenPanel'].getBoundingClientRect()
      )
    },
    ...
    methods: {
      ...
      roCallBack(entries, observer) {
        if (entries && entries[0] && entries[0].contentRect) {
          ...
          this.$store.dispatch('app/setScreenPanelRect', contentRect)
        }
      },
    },
  }
