  ...
  <div :class="isDetailShow ? 'detail-bid' : 'detail-bid special'">
    ...
    <div v-if="!isDetailShow" class="default-container">
      <div class="detail-default">
        ...
        <span>`该标讯已被删除或下架`</span>
      </div>
    </div>
    <!-- 标讯/采购意向详情 -->
    <div v-else class="main-container">
      <MuPageHeader title="标讯详情" :show-reload="false">
        <div class="detail-title-buttons-wrp">
          ...
          <template v-if="!isUnlock">
            ...
            <a-button
              v-if="
                detailInfo.operateDetail.isShowOrigin &&
                detailInfo.operateDetail.originAddress
              "
              @click="viewSource"
            >
              查看原文
      ...
      <Blur
        v-if="isUnlock"
        module="bid"
        src="https://img.mumuxili.com/yqmm/pc/saas/images/blur/bid.png"
            ...
            <div class="info-content">
              ...
              >
                ...
                <div v-if="detailInfo.baseInfo.bidCompany">
                  采购单位：
                  <span>{{ detailInfo.baseInfo.bidCompany }}</span>
                </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      copyShareLink() {
        const text = `${window.location.origin}/yqmmpro/detail/bid/${this.id}#${this.detailInfo.titleDetail.title}`
      ...
      // 获取文章详情
      async getDetail(id) {
        const res = await getBidDetail({ tokenEncodeId: id })
        if (res.code === '0') {
      ...
      handleBaseInfo(e) {
        ...
        if (e.winBidList && e.winBidList.length) {
          winBidList = e.winBidList.map(item => {
            let money = item.winBidMoneyNum
              ? '，' + item.winBidMoneyNum + '万元'
              : ''
        ...
        if (e.bidTypeId === 802) {
          ...
          Object.assign(this.detailInfo.baseInfo, {
            ...
            agencyContent: e.agencyContent
              ? e.agencyContent.replaceAll(';', '、')
              : '',
          ...
          Object.assign(this.detailInfo.baseInfo, {
            ...
            winBidContact: e.winBidContact
              ? e.winBidContact.replaceAll(';', '、')
              : '',
            winBidTel: e.winBidTel ? e.winBidTel.replaceAll(';', '、') : '',
            bidReviewer: e.bidReviewer
              ? e.bidReviewer.replaceAll(';', '、')
              : '',
            contractNo: '',
            contractName: '',
            contractDate: '',
