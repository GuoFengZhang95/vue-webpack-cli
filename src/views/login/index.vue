...
<script>
  import { queryAdvertisement } from '@/api/login.js'
  export default {
    name: 'Login',
    data() {
      return {
    ...
    created() {
      queryAdvertisement({
        ...
        if (res.data && res.data[0] && res.data[0].imgs) {
    ...
    mounted() {
      ...
      this.spinning = true
      this.src = `https://globaltpl${process.env.ENV}.mumuxili.com/login?registerPlatformId=8&returnUrl=${url}`
