...
class BundleAnalyzerPlugin {
  ...
  apply(compiler) {
    ...
    const done = (stats, callback) => {
      ...
      // fs.writeFileSync('stats.json', JSON.stringify(stats.toJson()), {encoding: 'utf8'})
      callback = callback || (() => { })
  ...
  async generateStatsFile(stats) {
    ...
    try {
      ...
      this.logger.info(
        // `${chalk.bold('Webpack Bundle Analyzer')} saved stats file to ${chalk.bold(statsFilepath)}`
      )
    } catch (error) {
      this.logger.error(
        `${chalk.bold('Webpack Bundle Analyzer')} error saving stats file to ${chalk.bold(statsFilepath)}: ${error}`
      )
    }
  }
